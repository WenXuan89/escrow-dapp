...
ETHERSCAN_KEY="your_etherscan_api_key"     # Optional for contract verification
   ```
4. Open the **Ganache** desktop app and start a new workspace. Note the RPC server address shown (default `127.0.0.1:7545`). Note the Chain ID (commonly 1337 or 5777).
...
**Agreement Creation**
- Route-aware agreement creation with origin, destination, item type, parcel size, weight, delivery speed and guarantee tier
- Live, non-binding ETH-to-MYR estimate for shippers using CoinGecko's public simple-price endpoint, with a short local cache if the service is temporarily unavailable
...
- Carrier milestone reporting, shipper verification and automated payout release
...
- Arbitrator-configurable completion and dispute-win rewards, limited to 1–500 points per reward

**Dispute Resolution**
- Dispute raising with predefined reasons and custom "Other" reason with description
- Written dispute information submission (Shipper/Carrier can submit; Arbitrator can view all)
...
- Combined milestone progress and chronological report/verification history
- Shipper and carrier activity feeds only show events for agreements that have reached Accepted or a later status; new carrier offers appear separately as notifications
- Arbitrator activity includes relevant agreement, dispute, refund and commission events
- ReputationToken point/star display for carriers
- Role-specific notifications for new offers, accepted agreements, funded work, reported milestones and active disputes
- Recent accepted agreements exclude offers that have not yet been accepted
